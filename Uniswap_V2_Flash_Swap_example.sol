...
contract UniswapV2FlashSwap is IUniswapV2Callee {
    ...
    // For this example, we are setting the amout to repay
...
// This function is called by WETH/DAI pair contract
function uniswapV2Call(
    address sender, 
    uint amount0, 
    uint amount1, 
    bytes calldata data
        ...
        require(msg.sender == address(pair), " Not address pair");
        require(sender == address(this), " Not sender");

        (address tokenBorrow, address caller) = abi.decode(data, (address , address));

        // Our custom code will go here, Example code to arbitrage
